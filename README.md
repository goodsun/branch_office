...
## HR/profiles/ JSONスキーマ

社員プロフィールのJSONフィールド定義。全フィールド必須。

| フィールド | 型 | 説明 | 例 |
|---|---|---|---|
| `name` | string | 英語名 | `"Teddy"` |
| `name_ja` | string | 日本語名 | `"テディ"` |
| `emoji` | string | 識別emoji | `"🧸"` |
| `role` | string | 役職 | `"AI Assistant / HQ"` |
| `division` | string | 事業部 | `"HQ"` |
| `hire_date` | string | 入社日（ISO 8601） | `"2026-02-07"` |
| `character` | string | キャラクター説明（日本語） | `"テディ（Teddy）: ..."` |
| `prompt_features` | string | 外見テキスト（英語、衣装を含まない） | `"anime girl with light brown bob-cut hair..."` |
| `charsheet` | string | デフォルトキャラシートパス（HR/相対） | `"HR/charsheets/teddy/teddy_main.jpg"` |
| `default_style` | string | デフォルトスタイルキー | `"normal"` |
| `styles` | object | スタイル定義（下記参照） | `{"normal": {...}}` |
| `personality` | object | 人格定義（下記参照） | `{"tone": "...", ...}` |
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `description` | ✅ | スタイルの説明（日本語） |
| `prompt_prefix` | ✅ | 画風指定テキスト |
| `model` | ✅ | 使用する画像生成モデル |
| `charsheet_override` | — | スタイル固有のキャラシートパス |
| `prompt_features_override` | — | スタイル固有の外見テキスト |
| `outfit` | — | 衣装テキスト（prompt_featuresに追記） |

> **重要:** `charsheet_override` と `prompt_features_override` は**必ずセットで定義**すること。片方だけでは外見が正しく切り替わらない。
...
|---|---|
| `tone` | 口調・性格 |
| `catchphrase` | 代表的なセリフ・emoji |
| `role_description` | 役割の詳細説明 |
